...
function activate(context) {
    ...
    toggleButton.show();
    context.subscriptions.push(vscode.commands.registerCommand('spotify-mini-player.toggle', () => {
        if (panel) {
            panel.dispose();
            panel = undefined;
        }
    ...
    }), trackItem, toggleButton);
    function updateTrack() {
        (0, child_process_1.exec)(`osascript -e 'tell application "Spotify"
    ...
    function openMiniPlayer() {
        panel = vscode.window.createWebviewPanel('spotifyMiniPlayer', '🎧 Spotify Mini Player', vscode.ViewColumn.Two, { enableScripts: true, retainContextWhenHidden: true });
        ...
        panel.webview.onDidReceiveMessage(msg => {
            if (msg.command === 'playpause')
                (0, child_process_1.exec)(`osascript -e 'tell application "Spotify" to playpause'`);
            else if (msg.command === 'next')
                (0, child_process_1.exec)(`osascript -e 'tell application "Spotify" to next track'`);
            else if (msg.command === 'prev')
                (0, child_process_1.exec)(`osascript -e 'tell application "Spotify" to previous track'`);
